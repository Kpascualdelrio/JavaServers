...
public class MySQLSocioDAO implements DaoSocio {
	...
	public void modificar(Socio socio) throws JAXBException {
		...
		try {
			...
			String nombreBase = jdbcTemplate.queryForObject("SELECT namePartner FROM socio WHERE id = ?;", String.class,
					socio.getIdPartner());

			// Asignamos el valor del Id, mediante el query a sql, del socio que nos
			// interese
			int socioId = jdbcTemplate.queryForObject("SELECT id FROM socio WHERE id = ?;", Integer.class,
					socio.getIdPartner());
			...
			jdbcTemplate.update("UPDATE socio SET namePartner = ? WHERE id = ?;", socio.getNamePartner(), socioId);
			...
			System.out
					.println("---------- El socio: " + nombreBase + " ahora se llama: " + socio.getNamePartner() + ".");

		} catch (DataIntegrityViolationException esql) {
			System.out.println("El nombre del socio " + socio.getNamePartner() + " ya existe.");
	...
	public void eliminar(Socio socio) throws JAXBException {
		try {
			...
			// Asignamos el valor del Id, mediante el query a sql, del socio que nos
			// interese
			int socioId = jdbcTemplate.queryForObject("SELECT id FROM socio WHERE namePartner = ?;", Integer.class,
					socio.getNamePartner());

			// Eliminamos la fila del id del socio guardada.
			jdbcTemplate.update("DELETE FROM socio WHERE id = ?;", socioId);
	...
	public void obtenerTodosSocio() {
		List<Map<String, Object>> rows = (List<Map<String, Object>>)
				jdbcTemplate.queryForList("SELECT * FROM socio ORDER BY id ASC");
